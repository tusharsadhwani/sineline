...
function draw() {
  ...
  if (!died && !leaderboard) {
    ...
    if (blocks[blocks.length - 1].y >= -height / 3) {
      // New block pushed each time the previous block reaches halfway through
      blocks.push(
        new Block(
          obj,
          mode,
  ...
  for (let i = trails.length - 1; i >= 0; i--) {
    if (trails[i].y > float(height) / 6 + trails[i].r) {
      // Deleting trails that get past the screen
      trails.splice(i, 1);
    }
  }

  for (let i = blocks.length - 1; i >= 0; i--) {
    if (blocks[i].y > float(height) / 6 + blocks[i].r) {
      // Deleting blocks that get past the screen
      blocks.splice(i, 1);
    }
  }
  ...
  text(int(score / 100), -width * 0.45, -height * 0.77); // Score on top
}
...
function scores() {
  ...
  for (let i = 1; i <= 5; i++) {
    if (highscores.length >= i) {
      text(
        highscores[highscores.length - i][0] +
          " - " +
          highscores[highscores.length - i][1],
        0,
        -height * (0.6 - 0.05 * i)
      );
    }
...
function Block(obj, mode, x, y, r = 0) {
  ...
  this.hits = function (arrow) {
    if (mode == "disc") {
      return (
        (this.x - arrow.x) ** 2 + this.y ** 2 < (this.r + width / 25.0) ** 2
      );
    } else {
      if (this.w > 0) {
        return (
          arrow.x > this.x &&
          arrow.y - width / 25.0 < this.y &&
          arrow.y + width / 25.0 > this.y - height / 15.0
        );
      } else {
        return (
          arrow.x < this.x &&
